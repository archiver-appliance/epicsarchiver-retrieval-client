...
@click.option('--method', default="MONITOR", type=click.Choice(["MONITOR", "SCAN"]),
...
def archive(ctx, period, method, files):
    ...
    pvs = list(itertools.chain([utils.parse_archive_file(filename, period, method) for filename in files]))
    if ctx.obj["debug"]:
